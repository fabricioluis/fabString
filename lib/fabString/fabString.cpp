...
char *strpart(char *str1, const char *str2, const char *str3) {
  ...
  if (posStr2_F == -1) return NULL;

  // Pega a posicao inicial da String 3, senao achou entao vai ateh o final de
  // str1.
  int posStr3_I = strpos(str1, str3, posStr2_F);
  if (posStr3_I == -1) posStr3_I = strlen(str1);

  if (posStr2_F > posStr3_I) return NULL;
...
//-------------------------------------------------------------------------------------------------------------------------//
/*
 * Funcao strpos (char *str1, char *str2, int n = -1)
 ...
 * n - se informado, indica que a ocorrencia deve ser a partir da posicao "n"
 * Senao encontrar, retorna -1.
 */
int strpos(char *str1, const char *str2, int n) {
  char *pos;

  if (n == -1)
    pos = strstr(str1, str2);
  else
    pos = strstr(str1 + n, str2);
...
char *substrend(char *str1, size_t pos) {
  ...
  for (size_t i = 0; i < tamRetorno; i++) *(retorno + i) = *(str1 + pos + i);
...
char *substr(char *str1, size_t pos, size_t tam) {
  ...
  for (size_t i = 0; i < tam; i++) *(retorno + i) = *(str1 + pos + i);
